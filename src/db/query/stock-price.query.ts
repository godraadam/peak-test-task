...
import { db } from "@/db";
import { type StockPriceCreate, stockPricesTable } from "@/db/schema/stock-prices.schema";
...
export async function getStockPricesByStockId({ stockId, limit, offset }: { stockId: string } & PaginationPayload) {
  return await db
    .select()
